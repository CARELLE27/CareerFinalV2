...
export default function Quetes() {
  ...
  <div className="valide-box">
    ...
    {/* Bouton LinkedIn si la quête a des compétences associées */}
    {selected.quete.competences_debloquees?.length > 0 && (
      <button
        className="linkedin-share-btn"
        style={{ marginTop: '14px', width: '100%' }}
        onClick={() => {
          const comp = selected.quete.competences_debloquees
            .map(c => c.nom).join(', ');
          const texte = `🎮 J'ai validé la quête "${selected.quete.titre}" sur CareerQuest et débloqué la compétence "${comp}" ! #CareerQuest #${comp.replace(/[\s\/]/g, '')} #Formation #Dev`;
          window.open(
