...
export default function CompetenceUnlockedModal({
  ...
  const [copied, setCopied] = useState(null); // index de la compétence copiée
  ...
  const handleShare = async (competence, index) => {
    const texte = queteTitre
      ...
      : `🎮 J'ai débloqué la compétence "${competence}" sur CareerQuest ! Niveau ${level} 🚀\n\n#CareerQuest #${competence.replace(/[\s\/\-]/g, '')} #Formation`;
  ...
  const S = {
    ...
    copiedMsg: { fontSize: '0.78rem', color: '#4ade80', marginTop: '4px', padding: '6px 10px', background: 'rgba(22,163,74,0.15)', border: '1px solid #16a34a', borderRadius: '6px' },
      ...
      <div style={S.box} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.linkedinSection}>
          <p style={S.linkedinLabel}>
            📢 Partagez votre réussite — le texte sera <strong style={{color:'#4ade80'}}>copié automatiquement</strong> :
          </p>
          {competences.map((c, i) => (
            <div key={i}>
              <button
              ...
              {/* ✅ Message après clic : texte copié */}
              {copied === i && (
                <div style={S.copiedMsg}>
                  ✅ Texte copié ! LinkedIn s'ouvre — collez avec <strong>Ctrl+V</strong> dans votre post 🎉
                </div>
              )}
            </div>
          ))}
